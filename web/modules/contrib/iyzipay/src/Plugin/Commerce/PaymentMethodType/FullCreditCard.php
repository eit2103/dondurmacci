...
class FullCreditCard extends PaymentMethodTypeBase {
  ...
  public function buildLabel(PaymentMethodInterface $payment_method) {
    ...
    // Log the decrypted card type for debugging.
    \Drupal::logger('iyzipay')->notice('Decrypted card type: @type', ['@type' => $decrypted_card_type]);

    try {
      $card_type = CreditCardHelper::getType($decrypted_card_type);
    } catch (\InvalidArgumentException $e) {
      // Log the exception message for debugging.
      \Drupal::logger('iyzipay')->error('Invalid card type: @message', ['@message' => $e->getMessage()]);
      throw $e;
    }
