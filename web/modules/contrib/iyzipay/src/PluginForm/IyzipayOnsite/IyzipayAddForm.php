...
class IyzipayAddForm extends PaymentGatewayFormBase {
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    try {
        ...
        \Drupal::logger('iyzipay')->notice('Creating payment method with details: <pre>@details</pre>', ['@details' => print_r($values['payment_details'], TRUE)]);

        // Create the payment method using the Iyzico API.
        $payment_gateway_plugin->createPaymentMethod($payment_method, $values['payment_details']);

        // Log the payment method details after creation.
        \Drupal::logger('iyzipay')->notice('Payment method created with remote ID: @remote_id', ['@remote_id' => $payment_method->getRemoteId()]);
        
        // Save the payment method locally.
        $payment_method->save();

        // Log the saved payment method ID for debugging.
        \Drupal::logger('iyzipay')->notice('Payment method saved with ID: @id', ['@id' => $payment_method->id()]);
    }
  ...
  protected function validateCreditCardForm(array &$element, FormStateInterface $form_state) {
    ...
    \Drupal::logger('iyzipay')->notice('Detected card type: @type', ['@type' => $card_type ? $card_type->getId() : 'none']);
    ...
    // Validate that the card type is one of the accepted by the payment gateway.
    $accepted_card_types = ["amex", "mastercard", "visa", "troy", "visa electron"];

    if ($accepted_card_types && !in_array($card_type->getId(), $accepted_card_types)) {
        $form_state->setError($element['number'], $this->t('The %card_type card type is not currently accepted.', ['%card_type' => $card_type->getLabel()]));
    }
  ...
  protected function submitCreditCardForm(array $element, FormStateInterface $form_state) {
    ...
    \Drupal::logger('iyzipay')->notice('Submitted card type: @type', ['@type' => $values['type']]);
  }
